<template>
  <div class="container">
    ...
    <n-space class="search">
      <n-input v-model:value="pageInfo.keyword"
               :style="{ width: '500px' }"
               placeholder="请输入关键字" />
      <n-button type="primary"
                ghost
                @click="loadBlogs(0)"> 搜索 </n-button>
    </n-space>
    ...
    <n-pagination @update:page="loadBlogs"
...
const loadBlogs = async (page = 0) => {
  ...
  let res = await axios.get(
    `/blog/search?keyword=${pageInfo.keyword}&page=${pageInfo.page}&pageSize=${
      pageInfo.pageSize
  ...
  for (let row of temp_rows) {
    console.log('row的值'+row.content)
    row.content += "...";
    // 把时间戳转换为年月日
    let d = new Date(row.create_time);
    row.create_time = `${d.getFullYear()}年${d.getMonth() +
      1}月${d.getDate()}日`;
  }
